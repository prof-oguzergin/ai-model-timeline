...
# ADIM 5: Bölge etiketlerini yeni-tarih (max) tarafına taşı, ha="right"
ax.text(df["Date"].max() - pd.Timedelta(days=30), y_positions["MiniMax"] + 0.5,
        "AÇIK KAYNAK / AÇIK AĞIRLIK",
        fontsize=56, color="#3fb950", va="center", ha="right",
        fontweight="bold", alpha=0.22)
ax.text(df["Date"].max() - pd.Timedelta(days=30), y_positions["Cohere"] + 0.5,
        "KAPALI KAYNAK",
        fontsize=56, color="#f778ba", va="center", ha="right",
        fontweight="bold", alpha=0.22)
...
# ADIM 4: Şirket etiket pozisyonlarını date_max + offset tarafa taşı
x_label_pos = date_max + pd.Timedelta(days=30)
...
for y_pos, company in labels_y:
    ...
    if sname:
        ax.text(x_label_pos, y_pos + 0.05, company_name[company],
                ...
                ha="right", va="center")
    # Country name - below, shifted to make room for flag
    country = country_text[company]
    flag_x_offset = date_max + pd.Timedelta(days=65)
    ...
    flag_date = date_max + pd.Timedelta(days=45)
...
# ADIM 3: X eksenini ters çevir (büyük değer önce)
ax.set_xlim(mdates.date2num(pd.Timestamp("2026-07-01")), mdates.date2num(pd.Timestamp("2022-11-01")))
...
plt.savefig("G:/My Drive/Claude Code/YZ Model Zaman Cizelgesi/ai_timeline_final_tr_reversed.png", dpi=150, bbox_inches="tight",
            pad_inches=1.0, facecolor="#0d1117", edgecolor="none")
print("Ters cevrilmis Turkce grafik kaydedildi! (ai_timeline_final_tr_reversed.png)")
